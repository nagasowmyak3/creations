...
$session->checkUserAccess("block_creation");
		...
		<div class="col-xl-12 col-lg-12">
			<div class="card border-0 shadow mb-4">
				<div
					...
					<h4 class="m-0 font-weight-bold text-primary">Block</h4>
				</div>
					...
					<div class="row">
						<div class="col-lg-3 col-md-12">
						    <div class="form-group row">
							  <label class="col-sm-12 col-form-label">Block number</label>
								<div class="col-sm-12">
								<input type="number" id="block_number" name="block_number" data-type="man" data-alias="number" class="form-control" maxlength="30" value="<?php echo $form->value("block_number"); ?>">
										<span class="error"><?php echo $form->error("block_number"); ?></span>
                            </div>
						</div>
						</div>
						<div class="col-lg-3 col-md-12">
						    <div class="form-group row">
							  <label class="col-sm-12 col-form-label">Block Name</label>
								<div class="col-sm-12">
								<input type="text" id="bname" name="bname" data-type="man" data-alias="Name" class="form-control" maxlength="30" value="<?php echo $form->value("bname"); ?>">
										<span class="error"><?php echo $form->error("bname"); ?></span>
                            </div>
                         </div>	
						 ...
						 <div class="form-group row mt-4 text-center">
							<div class="col-md-12 m-2">
								<button class="btn btn-md btn-success" type="button" onClick="setState('adminForm','<?php echo SECURE_PATH; ?>block_creation/process.php','validateForm=1'+sendData())">
									 Submit</button>
							</div>
						</div>
...
if(isset($_REQUEST['validateForm']))
{
	...
	if($form->num_errors > 0 || $post['validateForm'] == 2)
	{
		...
		setState('adminForm','<?php echo SECURE_PATH;?>block_creation/process.php','addForm=1<?php echo $return_var; ?><?php if(isset($_REQUEST['editform'])){ echo '&editform='.$post['editform'];}?>');
	...
	else
	{
		...
		$ins=$database->connection->prepare("INSERT INTO block_creation VALUES (NULL,:block_number,:block_name)");
        $ins->execute(array(
		'block_number' => $form->value('block_number'), 
	   'block_name'   => $form->value('bname'),
	...
	<script type="text/javascript">
		...
		setState('adminForm','<?php echo SECURE_PATH;?>block_creation/process.php','addForm=1<?php echo $return_var; ?><?php if(isset($_REQUEST['editform'])){ echo '&editform='.$post['editform'];}?>');
		setState('adminTable','<?php echo SECURE_PATH;?>block_creation/process.php','tableDisplay=1<?php echo $return_var; ?><?php if(isset($_REQUEST['editform'])){ echo '&editform='.$post['editform'];}?>');
...
if(isset($_REQUEST['tableDisplay']))
{
	...
	$pagename=SECURE_PATH.'block_creation/process.php';
	$method="tableDisplay"; 
	$pagination=showPagination(SECURE_PATH."block_creation/process.php?tableDisplay=1&",$tableName,$start,$limit,$page,$conditions);
